...
# scripts are used for testing purpose. However cmake makes a symlink
# from build/bin to scripts, so we have to copy scripts dir first
...
# run tests
cd ../test
./bloocoo_test.sh || error_code
./simple_test.sh || error_code
./test_ERR039477.sh  || error_code
./test_metag_SRS018585.sh || error_code
# cleanup disk space
cd ..
rm -rf test
